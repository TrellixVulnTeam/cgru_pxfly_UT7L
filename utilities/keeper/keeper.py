...
# ############################################################################
# init [AF_USERNAME] and [PWD]
# added by DavidPower
os.environ['AF_USERNAME'] = cgruconfig.VARS['USERNAME']
os.environ['PWD'] = 'C:\\cgru-pwd'
# ############################################################################

# ############################################################################
# check update
# added by DavidPower
if os.getenv('COMPUTERNAME').lower() != cgruconfig.VARS['CGRU_UPDATESERVER'].lower():
	cgru_location = os.path.basename(cgruconfig.VARS['CGRU_LOCATION'])
	server_verfile = '//' + cgruconfig.VARS['CGRU_UPDATESERVER'] + '/' + cgru_location + '/version.txt'
	verInfo = ''
	try:
		with open(server_verfile, 'r') as f:
			verInfo = f.read().strip()
	except:
		pass

	if verInfo > cgruconfig.VARS['CGRU_VERSION']:
		update_cmd = '"%s" "%s"' % (os.getenv('CGRU_PYTHONEXE'), os.path.dirname(sys.argv[0]) + '\\update.py')
		cgruconfig.VARS['CGRU_UPDATE_CMD'] = update_cmd
		os.environ['CGRU_UPDATE_CMD'] = update_cmd
	else:
