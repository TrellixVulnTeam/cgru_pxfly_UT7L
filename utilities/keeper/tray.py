...
class Tray(QtGui.QSystemTrayIcon):
	def __init__(self, parent=None):
		...
		# ############################################################################
		# show and change USERNAME
		# added by DavidPower
		# ############################################################################
		from functools import partial
		action = QtGui.QAction(cgruconfig.VARS['USERNAME'], self)
		action.setIcon(QtGui.QIcon(cgruutils.getIconFileName('user')))
		QtCore.QObject.connect(action, QtCore.SIGNAL('triggered()'),
								partial(cmd.setAFANASYUser, action))
		self.menu['menu'].addAction(action)
		self.menu['menu'].addSeparator()
		# ############################################################################
