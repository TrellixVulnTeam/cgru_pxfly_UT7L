...
class RenderOnFarm(plugins.CommandData):
	...
	def Execute(self, thisDoc):
		...
		# ############################################################################
		# For saving a temp render doc
		# changed by DavidPower

		# The Default-Path to export to
		self.tempSceneName = '.'.join(thisDoc.GetDocumentName().split('.')[:-1]) + '.' + str(
				int(time.time())) + '.c4d'
		...
		# get output path, this was originally null
		# changed by DavidPower
		self.output = self.thisDoc.GetActiveRenderData()[c4d.RDATA_PATH]
	...
	def startFarmRender(self):
		...
		if self.output != '':
			...
			# ############################################################################
			# Make extension, for enabling thumbnail and browse in afWatch
			# added by DavidPower
			oimageEXT = '@###'
			NAMEFORMAT = self.thisDoc.GetActiveRenderData()[c4d.RDATA_NAMEFORMAT]
			if NAMEFORMAT == 2 or NAMEFORMAT == 5 or NAMEFORMAT == 6:
				oimageEXT = '.' + oimageEXT
			if NAMEFORMAT == 0 or NAMEFORMAT == 1 or NAMEFORMAT == 2 or NAMEFORMAT == 6:
				oimageEXT = oimageEXT + '#@'
			else:
				oimageEXT = oimageEXT + '@'
			if NAMEFORMAT == 0 or NAMEFORMAT == 3 or NAMEFORMAT == 6:
				ext = ''
				FORMAT = self.thisDoc.GetActiveRenderData()[c4d.RDATA_FORMAT]
				if FORMAT == 1016606: ext = '.exr'
				if FORMAT == 1023737: ext = '.dpx'
				if FORMAT == 	1104: ext = '.jpg'
				if FORMAT == 1023671: ext = '.png'
				if FORMAT == 	1101: ext = '.tga'
				if FORMAT == 1100 or FORMAT == 1110: ext = '.tif'
				oimageEXT = oimageEXT + ext
			# ############################################################################

			# ############################################################################
			# Add extension to output image name, for enabling thumbnail and browse in afWatch
			# changed by DavidPower
			block.setFiles([self.output + oimageEXT])
			# ############################################################################
		...
		# When a temp-scene got created and has to be
		# deleted in the end
