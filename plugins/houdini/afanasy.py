...
# ############################################################################
# List of parameters to check for eval: added by DavidPower
param_watchList = ['vm_picture', 'soho_diskfile']
# ############################################################################
...
class BlockParameters:
	...
	def doPost(self):
		...
		# ############################################################################
		# restore param value after expend path expression
		# added by DavidPower
		# ############################################################################
		for param in param_watchList:
			if param in self.param_restoreList:
				self.ropnode.parm(param).set(self.param_restoreList[param])
		# ############################################################################
...
# ############################################################################
# expand output path expression for ROP nodes and replace $OS to afNode
# added by DavidPower
# ############################################################################
def expandOutputPath(params):
	for parm in params:
		if parm.ropnode is not None:
			for pm in parm.ropnode.parms():
				if pm.name() in param_watchList:
					if pm.eval() != '':
						exprString = pm.unexpandedString()
						if '$OS' in exprString:
							parm.param_restoreList[pm.name()] = exprString
							pm.set(exprString.replace('$OS', parm.afnode.name()))			
						pm.set(pm.eval())
# ############################################################################
